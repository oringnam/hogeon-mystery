...
## v4.15 (2026-02-15) - 🖼️ **이미지 매칭 오류 수정(스왑)**

### 변경
- 이미지 파일 매칭이 섞인 항목들을 올바른 엔딩으로 스왑
  - `ending_business.png` ↔ `ending_religion.png`
  - `ending_hospital.png` ↔ `ending_business.png`(이전 매칭 오류 정리)
  - `prologue.png` ↔ `ending_overseas.png`

### 기술
- Service Worker 캐시 버전 bump (v56)
...
### 변경
- 프롤로그 이미지(`prologue.png`)를 ChatGPT 생성 원본(클린 이미지) 기준으로 교체
- `reference-photos/` 내 남아있던 추적 파일을 git 인덱스에서 제거(로컬에는 남길 수 있음)

### 기술
- Service Worker 캐시 버전 bump (v53)

---

## v4.11 (2026-02-15) - 🖼️ **엔딩 일러스트 재캡처(클린 버전) + 사진 커밋 방지**

### 변경
- 엔딩 일러스트 일부를 ChatGPT 생성 원본(클린 이미지) 기준으로 교체
  - `ending_lotto.png`, `ending_ship.png`, `ending_isekai.png`, `ending_business.png`, `ending_hospital.png`, `ending_overseas.png`
- `ending_overseas.png` 중앙 정렬(좌우 여백) 재조정
- `reference-photos/` 내 실제 사진 파일 제거 + 향후 커밋 방지용 `.gitignore` 정리

### 기술
- Service Worker 캐시 버전 bump (v52)

---

## v4.10 (2026-02-15) - 🎨 **프롤로그 픽셀아트 스타일 개선**

### 변경
- 프롤로그 이미지(`prologue.png`)를 엔딩 일러스트와 더 비슷한 느낌(카페 배경 + 디테일/조명 + 픽셀 도트)으로 교체
  - 빈 자리(호건의 부재) + 돋보기/메모 모티프 유지

### 기술
- Service Worker 캐시 버전 bump (v51)

---

## v4.9 (2026-02-15) - 🏷️ **메인 화면 버전 자동 갱신**

### 변경
- 메인 화면 하단 버전 배지를 하드코딩 대신 `CHANGELOG.md`에서 자동 파싱하여 항상 최신 버전 표시
  - 네트워크 불가 시에는 “변경 이력 보기”로 fallback

### 기술
- Service Worker에서 `CHANGELOG.md`를 network-first로 처리 (최신 버전 반영 우선)
- Service Worker 캐시 버전 bump (v50)

---

## v4.8 (2026-02-15) - 🧹 **엔딩 모달/공유 문구 정리**

### 변경
- 엔딩 모달에서 “수집한 엔딩 n/9” 등 **진행률/도감 문구 제거**
- 공유하기 텍스트에서 “n/9개 엔딩 수집” 문구 제거

### 기술
- Service Worker 캐시 버전 bump (v49)
